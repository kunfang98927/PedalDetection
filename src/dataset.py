...
class PedalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # if "0" label is more than 50% of the sequence, skip this sample
        if self.split == "train":
            if np.sum(selected_label == 0) > self.max_frame * 0.3 or np.sum(selected_label == 127) > self.max_frame * 0.3:
                return self.__getitem__((idx + 1) % len(self))
