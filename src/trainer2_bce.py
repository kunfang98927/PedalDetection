...
class PedalTrainerBCE:
    ...
    def forward_for_one_batch(self, inputs, global_p_v_labels, p_v_labels, 
        ...
        p_v_loss = self.bce_criterion(p_v_logits.squeeze(), p_v_labels)
        ...
        global_p_v_loss = self.bce_criterion(
        ...
        with torch.no_grad():
            ...
            for (
                ...
                *_,
                ...
                # Pedal classification loss
                pedal_value_loss = self.bce_criterion(p_v_logits.squeeze(), p_v_labels)
                pedal_on_loss = self.bce_criterion(p_on_logits.squeeze(), p_on_labels)
                pedal_off_loss = self.bce_criterion(p_off_logits.squeeze(), p_off_labels)
                ...
                # Global pedal mse loss
                global_p_v_loss = self.bce_criterion(global_p_v_logits.squeeze(), global_p_v_labels)
                global_p_v_loss = global_p_v_loss.sum() / global_p_v_labels.shape[0]
                ...
                p_v_preds = torch.sigmoid(p_v_logits).squeeze()
                ...
                if global_pedal_ratio > 0:
                    ...
                    global_p_v_preds = torch.sigmoid(global_p_v_logits).squeeze()
