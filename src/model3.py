...
class PedalDetectionModelContrastive(nn.Module):
    ...
    def forward(self, x, loss_mask=None, src_mask=None, adv_lambda=1.0):
        # Apply CNN layers
        ...
        room_repr = self.room_projection(latent_repr)
        pedal_repr = self.pedal_projection(latent_repr)

        # Frame-wise Predictions
        p_v_logits = self.pedal_value_output_layer(pedal_repr.clone())
        ...
        if loss_mask is not None:
            ...
            room_repr *= loss_mask
            pedal_repr *= loss_mask

        # Mean Latent Representation for Global Predictions
        pedal_mean_latent_repr = pedal_repr.sum(dim=1) / loss_mask.sum(dim=1)
        room_mean_latent_repr = room_repr.sum(dim=1) / loss_mask.sum(dim=1)

        # Room Prediction uses only room_repr
        room_logits = getattr(self, "room_head", None)
        if room_logits is not None:
            room_logits = self.room_head(room_mean_latent_repr)
        else:
        ...
        if global_p_v_logits is not None:
            global_p_v_logits = self.global_pedal_value_head(pedal_mean_latent_repr)
        else:
            global_p_v_logits = torch.zeros_like(p_v_logits[:, :1])  # Placeholder tensor

        return global_p_v_logits, p_v_logits, p_on_logits, p_off_logits, room_logits, pedal_repr, pedal_mean_latent_repr
