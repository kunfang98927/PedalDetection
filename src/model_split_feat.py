...
class PedalDetectionModelwithCNNSplitFeat(nn.Module):
    def __init__(
        self,
        input_dim,
        hidden_dim,
        ...
        self.cnn = CNNBlock(hidden_dim=hidden_dim - 20, dropout=dropout) # minus 20 MFCC features
    ...
    def forward(self, x, loss_mask=None, src_mask=None):
        ...
        # split input tensor into audio and metadata
        x_mel = x[:, :, :229]
        x_mfcc = x[:, :, 229:]
        assert x_mel.shape[-1] == 229
        assert x_mfcc.shape[-1] == 20

        # Apply CNN layers
        x_mel = self.cnn(x_mel)  # [batch, seq_len, hidden_dim]
