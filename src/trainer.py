...
class PedalTrainer:
    ...
    def train(self, low_res_pedal_ratio=0.5, pedal_value_ratio=0.7, pedal_onset_ratio=0.1, pedal_offset_ratio=0.1,
        ...
        for epoch in range(self.num_train_epochs):
            ...
            val_loss, val_pedal_value_f1, val_pedal_on_mse, val_pedal_on_mae, val_pedal_on_f1, \
                val_pedal_off_mse, val_pedal_off_mae, val_pedal_off_f1, val_room_f1 = self.validate(
                epoch, pedal_value_ratio, pedal_onset_ratio, pedal_offset_ratio, room_ratio, contrastive_ratio
            )
            print(
                f"Epoch {epoch + 1}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Val Pedal Value F1: {val_pedal_value_f1:.4f}, "
    ...
    def train_one_epoch(self, epoch, low_res_pedal_ratio=0.5, pedal_value_ratio=0.7, 
        ...
        for batch_idx, (inputs, low_res_p_v_labels, p_v_labels, p_on_labels, p_off_labels, room_labels, midi_ids, pedal_factors) in enumerate(self.train_dataloader):
            inputs, low_res_p_v_labels, p_v_labels, p_on_labels, p_off_labels = inputs.to(self.device), p_v_labels.to(self.device), p_on_labels.to(self.device), p_off_labels.to(self.device)
            ...
            low_res_p_v_loss = self.criterion(low_res_p_v_labels)
    ...
    def validate(self, epoch, low_res_pedal_ratio=0.5, pedal_value_ratio=0.7, 
        ...
        with torch.no_grad():
            for inputs, low_res_p_v_labels, p_v_labels, p_on_labels, p_off_labels, room_labels, midi_ids, pedal_factors in self.val_dataloader:
                ...
                low_res_p_v_preds = torch.argmax(low_res_p_v_outputs, dim=-1)
        ...
        return val_loss / len(self.val_dataloader), avg_low_res_pedal_value_f1, avg_pedal_value_f1, \
            avg_pedal_onset_mse, avg_pedal_onset_mae, avg_pedal_onset_f1, \
            avg_pedal_offset_mse, avg_pedal_offset_mae, avg_pedal_offset_f1, \
            avg_room_f1
